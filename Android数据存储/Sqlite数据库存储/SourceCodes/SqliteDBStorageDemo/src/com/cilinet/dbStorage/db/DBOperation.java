...
//		mSqLiteDatabase.execSQL(_sql);
	...
	public boolean updateUserByAPI(){
		...
		int rowId = mSqLiteDatabase.update("users", _contentValues, "userId=?", new String[]{String.valueOf(2)});
	...
	/** 批量添加人员 **/
	public boolean insertUsers(List<Person> persons){
		...
		try{
			for(Person _person : persons){
				...
				_contentValues.put("userName", _person.personName);
				_contentValues.put("userAddress", _person.personName);
				
				//一次只能添加一条数据，即一个person
				mSqLiteDatabase.insert("users", null, _contentValues);
			}
